...
module {
  ...
  public func new<K, V>(seed : (Nat64, Nat64)) : HashMap<K, V> {
    { hamt = Hamt.new(); var size = 0; seed };
  };

  public func singleton<K, V>(seed : (Nat64, Nat64), hashFn : HashFn<K>, key : K, value : V) : HashMap<K, V> {
    let hashed = hashFn.0(seed, key);
    { hamt = Hamt.singleton(hashed, { var items = List.make((key, value)) }); var size = 1; seed };
  };

  public func fromIter<K, V>(seed : (Nat64, Nat64), hashFn : HashFn<K>, iter : Iter.Iter<(K, V)>) : HashMap<K, V> {
